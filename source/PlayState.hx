...
class PlayState extends MusicBeatState
{
		...
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'circle':
				{
					...
					babyArrow.animation.addByPrefix('pressed', 'circle ' + lowerDir + ' press', 24, false);
					babyArrow.animation.addByPrefix('confirm', 'circle1 ' + lowerDir + ' confirm', 24, false);

					for (i in 0...4)
					{
						babyArrow.animation.addByPrefix('static', 'circleStatic');
					}

					babyArrow.x += Note.swaggierWidth * i;
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (FlxG.save.data.cpuStrums)
					{
						...
						cpuStrums.forEach(function(sprCircle:FlxSprite)
						{
							if (sprCircle.animation.curAnim.name == 'confirm' && noteTypeCheck == 'circle')
							{
								sprCircle.centerOffsets();
								sprCircle.offset.x -= 8;
								sprCircle.offset.y -= 8.5;
							}
						});
					}
	...
	override function beatHit()
	{
		...
		if (FlxG.save.data.camzoom)
		{
			...
			if (curSong.toLowerCase() == 'tutorial' && camZooming && FlxG.camera.zoom < 1.35 && curBeat % 4 == 0)
			{
				FlxG.camera.zoom += 0.015;
				camHUD.zoom += 0.03;
			}
