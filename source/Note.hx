...
class Note extends FlxSprite
{
	...
	public static var swaggierWidth:Float = 175 * 0.7;
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false)
	{
		...
		else
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'circle': 
					...
					for (i in 0...4)
					{
						animation.addByPrefix(dataColor[i] + 'Scroll', dataColor[i] + ' circle');
		...
		if (noteTypeCheck == 'circle')
		{
			x += swaggierWidth * noteData;
		}
		else
		{
